...
class EndpointModelTrainer:
    ...
    def train(self, data_path: str, output_dir: str = None) -> Dict[str, Any]:
        ...
        # Evaluate model (for AE, pass output_dir and train data for threshold plot + FP minimization)
        ...
        # Persist reconstruction error stats and threshold metadata for inference-time detection (AE only)
        if self.model_type == "autoencoder" and metrics:
            for k in (
                "reconstruction_error_mean", "reconstruction_error_std", "anomaly_threshold", "k_std", "anomaly_rate",
        ...
        if self.model_type == "autoencoder":
            print(f"   Reconstruction mean: {metrics.get('reconstruction_error_mean', 0):.6f}")
    ...
    def _train_autoencoder(self, X_train: np.ndarray, y_train: np.ndarray, plot_dir: str | None = None):
        ...
        x = Dropout(drop)(x)
        x = Dense(encoding_dim * 2, activation="relu", kernel_regularizer=reg)(x)
        x = Dropout(drop)(x)
    ...
    def _train_lstm(self, X_train: np.ndarray, y_train: np.ndarray, plot_dir: str | None = None):
        ...
        if plot_dir:
            try:
                ...
                print(f"⚠️  Could not save LSTM history plot: {_e}")
    ...
    def _plot_ae_reconstruction_distribution(
        ...
        *,
        ...
        ax.set_xlabel("Reconstruction error (MSE)", fontsize=20, color="#374151")
    ...
    def _evaluate_model(
        ...
        if self.model_type == "autoencoder":
            ...
            print(f"      candidates      : p95={candidates.get('p95', 0):.6f} p97={candidates.get('p97', 0):.6f} p99={candidates.get('p99', 0):.6f} mean+{k_std}*std={candidates.get('mean_k_std', 0):.6f}")
